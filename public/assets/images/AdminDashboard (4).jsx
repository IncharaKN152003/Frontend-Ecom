        ...
        {activeTab==="payments" && (
          <div>
            <div style={{marginBottom:20}}>
              ...
              <p style={{fontSize:13,color:W.textSec,margin:0}}>All payments received — partner listing fees and student course enrollments</p>
            </div>

            {/* ── Stats row: 4 cards ── */}
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:24}}>
              {[
                { label:"Total Revenue",       value:`₹${payments.reduce((a,p)=>a+(+p.amount),0).toLocaleString("en-IN")}`,                                         bg:W.greenBg, color:W.greenText, border:"#BBF7D0" },
                { label:"Student Payments",    value:payments.filter(p=>p.type==="student_enrollment").length,                                                        bg:W.blueBg,  color:W.blueText,  border:"#BFDBFE" },
                { label:"Partner Payments",    value:payments.filter(p=>p.type==="partner_listing").length,                                                           bg:"#F5F3FF",  color:"#7C3AED",   border:"#DDD6FE" },
                { label:"Awaiting Payment",    value:thirdParty.filter(tp=>tp.status==="approved"&&tp.payment_status!=="paid").length,                                bg:W.amberBg, color:W.amberText, border:"#FDE68A" },
              ].map(s=>(
            ...
            {/* ── Payment table ── */}
            <div style={{background:W.white,border:`1px solid ${W.border}`,borderRadius:12,overflow:"hidden"}}>
              <div style={{padding:"16px 20px",borderBottom:`1px solid ${W.border}`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <p style={{fontSize:14,fontWeight:600,margin:0,color:W.textPrimary}}>Payment History</p>
                {payments.length>0 && (
                  <div style={{display:"flex",gap:6}}>
                    <span style={{fontSize:11,padding:"3px 10px",borderRadius:20,background:W.blueBg,color:W.blueText,fontWeight:600}}>
                      👨‍🎓 {payments.filter(p=>p.type==="student_enrollment").length} students
                    </span>
                    <span style={{fontSize:11,padding:"3px 10px",borderRadius:20,background:"#F5F3FF",color:"#7C3AED",fontWeight:600}}>
                      🏢 {payments.filter(p=>p.type==="partner_listing").length} partners
                    </span>
                  </div>
                )}
              </div>
              {payments.length===0 ? (
                <div style={{padding:48,textAlign:"center"}}>
                  ...
                  <p style={{fontSize:14,color:W.textSec}}>No payments recorded yet.</p>
                  <p style={{fontSize:12,color:W.textHint,marginTop:4}}>Payments appear here once students enroll in courses or partners pay listing fees.</p>
                </div>
              ) : (
                <>
                  <div style={{display:"grid",gridTemplateColumns:"2fr 1.5fr 0.8fr 1fr 1fr 1.2fr 0.9fr",padding:"10px 20px",background:W.grayBg,borderBottom:`1px solid ${W.border}`}}>
                    {["Course","Payer","Type","Amount","Method","Transaction ID","Date"].map(h=>(
                      <span key={h} style={{fontSize:11,color:W.textSec,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.06em"}}>{h}</span>
                    ))}
                  </div>
                  {payments.map((pay,i)=>{
                    const isStudent = pay.type === "student_enrollment";
                    const methodLabel = pay.method==="card"?"💳 Card":pay.method==="upi"?"📱 UPI":pay.method==="nb"?`🏦 ${pay.bank||"NetBanking"}`:pay.method==="razorpay"?"📲 Razorpay":"💳 Card";
                    const methodBg    = pay.method==="card"?W.blueBg:pay.method==="upi"?W.greenBg:"#FFF7ED";
                    const methodColor = pay.method==="card"?W.blueText:pay.method==="upi"?W.greenText:"#C2410C";
                    return (
                      <div key={pay.txnId||i} style={{display:"grid",gridTemplateColumns:"2fr 1.5fr 0.8fr 1fr 1fr 1.2fr 0.9fr",padding:"14px 20px",borderTop:i===0?"none":`1px solid ${W.border}`,alignItems:"center",background:W.white}}>
                        {/* Course */}
                        <div>
                          <p style={{fontSize:13,fontWeight:500,margin:0,color:W.textPrimary,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{pay.courseName}</p>
                          {pay.instructor&&<p style={{fontSize:11,color:W.textHint,margin:"2px 0 0"}}>{pay.instructor}</p>}
                        </div>
                        {/* Payer */}
                        <div>
                          <p style={{fontSize:13,margin:0,color:W.textPrimary,fontWeight:500}}>{pay.company}</p>
                          <p style={{fontSize:11,color:W.textHint,margin:"2px 0 0"}}>{pay.email}</p>
                        </div>
                        {/* Type badge */}
                        <span style={{fontSize:10,padding:"3px 8px",borderRadius:20,fontWeight:700,background:isStudent?W.blueBg:"#F5F3FF",color:isStudent?W.blueText:"#7C3AED",whiteSpace:"nowrap"}}>
                          {isStudent?"👨‍🎓 Student":"🏢 Partner"}
                        </span>
                        {/* Amount */}
                        <span style={{fontSize:14,fontWeight:700,color:W.greenText}}>₹{(+pay.amount).toLocaleString("en-IN")}</span>
                        {/* Method */}
                        <div>
                          <span style={{fontSize:11,padding:"3px 9px",borderRadius:20,fontWeight:500,background:methodBg,color:methodColor}}>{methodLabel}</span>
                          {pay.maskedCard&&<p style={{fontSize:10,color:W.textHint,margin:"3px 0 0"}}>{pay.maskedCard}</p>}
                          {pay.upiId&&<p style={{fontSize:10,color:W.textHint,margin:"3px 0 0"}}>{pay.upiId}</p>}
                        </div>
                        {/* Txn ID */}
                        <span style={{fontSize:10,fontFamily:"monospace",color:W.greenText,fontWeight:600,wordBreak:"break-all"}}>{pay.txnId}</span>
                        {/* Date */}
                  ...
                  {/* Footer totals */}
                  <div style={{padding:"14px 20px",background:W.grayBg,borderTop:`2px solid ${W.border}`,display:"grid",gridTemplateColumns:"2fr 1.5fr 0.8fr 1fr 1fr 1.2fr 0.9fr",alignItems:"center"}}>
                    <span style={{fontSize:13,fontWeight:700,color:W.textPrimary,gridColumn:"1/4"}}>Total Revenue</span>
                    <span style={{fontSize:15,fontWeight:700,color:W.greenText}}>₹{payments.reduce((a,p)=>a+(+p.amount),0).toLocaleString("en-IN")}</span>
                  </div>
                </>
              )}
            </div>
