...
const MOCK_QUERIES = [
  { id:1, user:"Kavya Reddy", email:"kavya@email.com", subject:"Certificate not received",       message:"I completed the React course 2 weeks ago but still haven't received my certificate. Please help.", status:"open",     date:"2025-03-20", priority:"high",   reply:"" },
  { id:2, user:"Amit Singh",  email:"amit@email.com",  subject:"Video not loading in Module 4",  message:"The video in Node.js Backend course Module 4 is not loading at all. I've tried multiple browsers.", status:"open",     date:"2025-03-21", priority:"medium", reply:"" },
  { id:3, user:"Divya Menon", email:"divya@email.com", subject:"Wrong quiz answers marked",      message:"In the UI/UX quiz, question 7's correct answer is marked wrong. Please review.",                    status:"resolved", date:"2025-03-15", priority:"low",    reply:"Thank you for reporting. We have corrected the quiz answer. Your score has been updated." },
  { id:4, user:"Rohan Gupta", email:"rohan@email.com", subject:"Payment deducted no enrollment", message:"I paid for Data Science Bootcamp but I'm not enrolled. Transaction ID: TXN2025031988.",             status:"open",     date:"2025-03-22", priority:"high",   reply:"" },
  { id:5, user:"Nisha Verma", email:"nisha@email.com", subject:"Request for refund",             message:"The course content is outdated. I'd like a refund as the description mentioned 2024 updates.",      status:"open",     date:"2025-03-23", priority:"medium", reply:"" },
];
...
export default function AdminDashboard() {
  ...
  const handleReply = (qid) => {
    ...
    setQueries(queries.map(q=>q.id===qid?{...q,status:"resolved",reply:replyText}:q));
    setReplyText(""); setSelectedQuery(null);
    showToast("Reply sent & query resolved!");
  };
        ...
        {activeTab==="queries" && (
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:selectedQuery?"1fr 1fr":"1fr",gap:16}}>
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {queries.map(q=>(
                  <div key={q.id} onClick={()=>{setSelectedQuery(q);setReplyText("");}} style={{background:W.white,border:selectedQuery?.id===q.id?`2px solid ${W.blue}`:`1px solid ${W.border}`,borderRadius:10,padding:"14px 16px",cursor:"pointer"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                      ...
                      <div style={{display:"flex",gap:6}}>{badge(q.priority,q.priority)}{badge(q.status,q.status)}</div>
                    </div>
                    ...
                    <p style={{fontSize:11,color:W.textHint,margin:"5px 0 0"}}>{q.date}</p>
                  </div>
                ))}
              </div>
              {selectedQuery && (
                <div style={{background:W.white,border:`1px solid ${W.border}`,borderRadius:12,padding:22,height:"fit-content"}}>
                  ...
                  {selectedQuery.reply && (
                    <div style={{background:W.greenBg,border:"1px solid #BBF7D0",borderRadius:8,padding:12,marginBottom:16}}>
                      <p style={{fontSize:11,color:W.greenText,fontWeight:600,margin:"0 0 4px"}}>Your reply — resolved</p>
                      <p style={{fontSize:13,color:W.green,margin:0}}>{selectedQuery.reply}</p>
                    </div>
                  )}
                  {selectedQuery.status==="open" && (
                    <div>
                      <label style={{fontSize:12,color:W.textSec,display:"block",marginBottom:6}}>Reply to {selectedQuery.user}</label>
                      ...
                      <button onClick={()=>handleReply(selectedQuery.id)} style={{marginTop:10,background:W.blue,color:"#fff",border:"none",borderRadius:8,padding:"10px 20px",fontSize:13,fontWeight:500,cursor:"pointer",width:"100%"}}>Send Reply & Mark Resolved</button>
                    </div>
                  )}
                </div>
              )}
